...
import { Globe, MapPin, Calendar, Users, Banknote, Phone, CheckCircle, Clock, ArrowLeft, Languages } from 'lucide-react';
import { fetchGuideBookings, acceptGuideBooking, completeGuideBooking } from '../lib/backendApi';
...
export default function GuideDashboard({ onBack }: Props) {
  ...
  const filteredBookings = bookings.filter(b => {
    // Фильтр по языку (если выбран)
    if (languageFilter !== 'all' && b.guide_id !== 'test-guide-456') {
      // В реальном приложении здесь была бы проверка языка гида
    }
    
    if (activeTab === 'new') return !b.guide_id && b.status !== 'completed' && b.status !== 'accepted';
    if (activeTab === 'my') return b.guide_id === 'test-guide-456' && b.status === 'accepted';
    if (activeTab === 'completed') return b.status === 'completed' && b.guide_id === 'test-guide-456';
    return true;
  });

  const tabs = [
    { id: 'new', label: 'Новые', count: bookings.filter(b => !b.guide_id && b.status !== 'completed').length },
    { id: 'my', label: 'Мои активные', count: bookings.filter(b => b.guide_id === 'test-guide-456' && b.status === 'accepted').length },
    { id: 'completed', label: 'Завершённые', count: bookings.filter(b => b.status === 'completed' && b.guide_id === 'test-guide-456').length },
  ];

  const languages = ['Английский', 'Немецкий', 'Китайский', 'Японский', 'Корейский'];
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Добро пожаловать, Айгерим Т.</p>
        </div>
      ...
      <div className="mb-6 flex items-center gap-2 flex-wrap">
        ...
        <button
          ...
          className={`px-3 py-1 text-xs font-medium rounded-full transition-colors ${
            languageFilter === 'all' ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
          }`}
        >
          Все
        </button>
        {languages.map(lang => (
          <button
            ...
            className={`px-3 py-1 text-xs font-medium rounded-full transition-colors ${
              languageFilter === lang ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
            {lang}
          </button>
