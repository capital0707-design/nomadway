...
import { Globe, MapPin, Calendar, Users, Banknote, Phone, CheckCircle, Clock, ArrowLeft, Languages, LogOut } from 'lucide-react';
import { fetchGuideBookings, acceptGuideBooking, completeGuideBooking, login, logout } from '../lib/backendApi';
...
export default function GuideDashboard({ onBack }: Props) {
  const [user, setUser] = useState<{ id: string; name: string; phone: string } | null>(() => {
    try {
      const raw = localStorage.getItem('guide_user');
  ...
  const loadBookings = async () => {
    try {
      setLoading(true);
      const data = await fetchGuideBookings();
      setBookings(data);
    } catch (err: any) {
      console.error(err);
    } finally {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoginError('');
    try {
      const u = await login('guide', phone, password);
      setUser(u);
      setPhone('');
      setPassword('');
    } catch (err: any) {
  ...
  const handleAccept = async (bookingId: string) => {
    ...
    try {
      await acceptGuideBooking(bookingId);
      await loadBookings();
    } catch (err) {
  ...
  const handleComplete = async (bookingId: string) => {
    if (!confirm('Подтвердить завершение работы гидом?')) return;
    setActionLoading(bookingId);
    try {
      await completeGuideBooking(bookingId);
      await loadBookings();
    } catch (err) {
    ...
    return (
      <div className="max-w-md mx-auto px-4 py-16">
        ...
        <form onSubmit={handleLogin} className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
          ...
          <h1 className="text-xl font-bold text-gray-900 mb-1">Кабинет гида</h1>
          <p className="text-sm text-gray-500 mb-6">Вход для зарегистрированных гидов-переводчиков</p>
          ...
          <input
            ...
            className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-accent-500/20 focus:border-accent-400 mb-4"
            required
          />
          ...
          <input
            ...
            placeholder="••••••••"
            className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-accent-500/20 focus:border-accent-400 mb-3"
            required
          />
          ...
          <button
            type="submit"
            disabled={loggingIn}
            className="w-full bg-gradient-to-r from-accent-600 to-accent-700 disabled:from-gray-400 disabled:to-gray-500 text-white font-semibold py-3 rounded-xl shadow-lg shadow-accent-500/25 transition-all active:scale-[0.98]"
          >
  ...
  const filteredBookings = bookings.filter(b => {
    if (activeTab === 'new') return (!b.guide_id || b.guide_id !== user.id) && b.status !== 'completed';
    if (activeTab === 'my') return b.guide_id === user.id && b.status === 'accepted';
    if (activeTab === 'completed') return b.guide_id === user.id && b.status === 'completed';
    return true;
  });

  const tabs = [
    { id: 'new', label: 'Новые', count: bookings.filter(b => (!b.guide_id || b.guide_id !== user.id) && b.status !== 'completed').length },
    { id: 'my', label: 'Мои активные', count: bookings.filter(b => b.guide_id === user.id && b.status === 'accepted').length },
    { id: 'completed', label: 'Завершённые', count: bookings.filter(b => b.guide_id === user.id && b.status === 'completed').length },
  ];
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Панель гида-переводчика</h1>
          <p className="text-sm text-gray-500">Добро пожаловать, {user.name}</p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={loadBookings} className="text-sm text-accent-600 hover:text-accent-700 font-medium">
            🔄 Обновить
          </button>
        ...
        {tabs.map(tab => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors relative ${
              activeTab === tab.id ? 'text-accent-700 bg-accent-50' : 'text-gray-500 hover:text-gray-700'
            }`}
          >
            {tab.label}
            {tab.count > 0 && (
              <span className={`ml-1.5 px-1.5 py-0.5 text-xs rounded-full ${
                activeTab === tab.id ? 'bg-accent-200 text-accent-800' : 'bg-gray-200 text-gray-600'
              }`}>
                {tab.count}
              </span>
            )}
            {activeTab === tab.id && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-accent-600" />}
          </button>
        ))}
      </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                <div className="flex-1 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-4 text-sm text-gray-600 pt-2">
                    ...
                    {booking.guide_price > 0 && (
                      <span className="flex items-center gap-1.5 text-accent-600"><Globe className="w-4 h-4" /> +{booking.guide_price} сом (гид)</span>
                    )}
                  </div>
                  {booking.user_phone && (
                ...
                <div className="flex sm:flex-col gap-2 sm:min-w-[140px]">
                  {activeTab === 'new' && (
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-2 bg-accent-600 hover:bg-accent-700 disabled:opacity-50 text-white text-sm font-semibold py-2.5 px-4 rounded-lg transition-colors"
                    >
